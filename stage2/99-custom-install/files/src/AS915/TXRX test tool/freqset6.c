...
void set_low_pass_filter(uint8_t mixer, uint8_t highLow)
{
    ...
    gpiod_line_set_value(mixer == MIXER_TX ? tx_lpf_line : rx_lpf_line, highLow);
}
...
void printUsage(){
    printf("usage: freqset6 -chn=0~%d -idd=0~7(default:5)\n"
            ...
            "       5:  TX(%.2f), RX(%.2f)\n",
            MAX_FSK_CHAN_NUM - 1,
            FSK_CH6_TX_1, FSK_CH6_RX_1,
            FSK_CH6_TX_2, FSK_CH6_RX_2,
            FSK_CH6_TX_3, FSK_CH6_RX_3,
            FSK_CH6_TX_4, FSK_CH6_RX_4,
            FSK_CH6_TX_5, FSK_CH6_RX_5,
            FSK_CH6_TX_6, FSK_CH6_RX_6
        );
}
